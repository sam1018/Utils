...
namespace UnitTest
{
	...
	{
	public:
		TEST_METHOD(test_xml_test_small_customer_orders)
		{
			ptree tree;
			read_xml("../UnitTest/CustomerOrders_small.xml", tree);

			auto in = make_intros_object<customer_orders>(tree);

			auto res = get_actual_data();

			Assert::IsTrue(in == res);
		...
		TEST_METHOD(test_xml_test_customer_orders)
		{
			ptree tree;
			read_xml("../UnitTest/CustomerOrders.xml", tree);
